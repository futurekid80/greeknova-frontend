...
export default function StockDeepDive({ params }: { params: { symbol: string } }) {
  ...
  async function fetchData() {
    setLoading(true)
    try {
      const res = await fetch(`https://api.greeknova.com/stock-oi/${symbol}`)
      const json = await res.json()
      setData(json)
    } catch (e) { console.error(e) }
    try {
      const hres = await fetch(`https://api.greeknova.com/oi-history/${symbol}`)
      const hjson = await hres.json()
      setHistory(hjson)
      if (hjson.strikes?.length) setSelectedStrike(hjson.strikes[Math.floor(hjson.strikes.length/2)].strike)
    } catch (e) { console.error(e) }
    try {
      const pres = await fetch(`https://api.greeknova.com/pcr-trend/${symbol}`)
