...
export default function GammaSqueeze() {
  ...
  const [ivFilter, setIvFilter] = useState<'ANY' | 'HIDE_RICH' | 'CHEAP_ONLY'>('ANY')
  ...
  const filtersActive = searchTerm !== '' || regimeFilter !== 'ALL' || stageFilter !== 'ALL' || maxWallDist < 100 || confirmedOnly || ivFilter !== 'ANY'

  const passesIvFilter = (r: GexRow) => {
    if (ivFilter === 'HIDE_RICH') return r.iv_regime !== 'RICH'
    if (ivFilter === 'CHEAP_ONLY') return r.iv_regime === 'CHEAP'
    return true
  }
  ...
  const filteredWatchlist = useMemo(() => {
    return watchlist.filter(r => {
      ...
      if (!passesIvFilter(r)) return false
      return true
    })
  }, [watchlist, searchTerm, regimeFilter, stageFilter, maxWallDist, confirmedOnly, ivFilter])
  ...
  useEffect(() => { setPage(1) }, [searchTerm, regimeFilter, stageFilter, maxWallDist, confirmedOnly, ivFilter, sortCol, sortDir])
  ...
  }, [signals, searchTerm, regimeFilter, stageFilter, confirmedOnly, ivFilter])
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
                        ...
                        <td className="px-3 py-2">
                          {r.pct_to_flip !== null ? (
                            <span className={`font-bold ${Math.abs(r.pct_to_flip) <= 1.5 ? 'text-yellow-400' : 'text-gray-400'}`}>
                              {r.pct_to_flip > 0 ? '+' : ''}{r.pct_to_flip.toFixed(1)}%
                            </span>
                          ) : (
        ...
        {!loading && !error && watchlist.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mb-6 bg-gray-900/20 border border-gray-800/40 rounded-xl px-3 py-2.5">
            ...
            <div className="flex items-center gap-1 bg-gray-900/60 border border-gray-800 rounded-lg p-0.5">
              {([
                { key: 'ANY', label: 'Any IV' },
                { key: 'HIDE_RICH', label: '⚠️ Hide rich' },
                { key: 'CHEAP_ONLY', label: '💎 Cheap only' },
              ] as const).map(opt => (
                <button key={opt.key} onClick={() => setIvFilter(opt.key)}
                  title={
                    opt.key === 'HIDE_RICH' ? 'Hide names where ATM IV is running well above realized vol — likely event risk / expensive premium rather than a clean mechanical squeeze'
                    : opt.key === 'CHEAP_ONLY' ? 'Show only names where ATM IV is at or below realized vol — the mechanically clean setup, without an inflated premium'
                    : undefined
                  }
                  className={`px-2.5 py-1 rounded-md text-[11px] font-semibold transition-all whitespace-nowrap ${
                    ivFilter === opt.key
                      ? opt.key === 'HIDE_RICH' ? 'bg-amber-900/60 text-amber-300' : opt.key === 'CHEAP_ONLY' ? 'bg-sky-900/60 text-sky-300' : 'bg-gray-700 text-white'
                      : 'text-gray-500 hover:text-gray-300'
                  }`}>
                  {opt.label}
                </button>
